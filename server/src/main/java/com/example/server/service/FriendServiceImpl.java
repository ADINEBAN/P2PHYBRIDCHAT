...
public class FriendServiceImpl extends UnicastRemoteObject implements FriendService {
    ...
    // 1. LẤY DANH SÁCH BẠN BÈ (ĐÃ SỬA: LẤY THÊM NICKNAME)
    @Override
    public List<UserDTO> getFriendList(long userId) throws RemoteException {
        List<UserDTO> list = new ArrayList<>();

        // [SỬA] Thêm f.nickname vào câu SELECT
        String sqlFriends = "SELECT u.id, u.username, u.display_name, u.is_online, u.last_ip, u.last_port, u.avatar_url, u.status_msg, f.nickname " +
                "FROM users u " +
        ...
        try (Connection conn = Database.getConnection();
            ...
            while (rs.next()) {
                ...
                // --- [LOGIC QUAN TRỌNG] ƯU TIÊN HIỂN THỊ BIỆT DANH ---
                String realName = rs.getString("display_name");
                String nickname = rs.getString("nickname");

                if (nickname != null && !nickname.trim().isEmpty()) {
                    u.setDisplayName(nickname); // Dùng biệt danh
                } else {
                    u.setDisplayName(realName); // Dùng tên thật
                }
                // ------------------------------------------------------
    ...
    // [MỚI] HÀM CẬP NHẬT BIỆT DANH BẠN BÈ (CHAT 1-1)
    @Override
    public boolean updateFriendNickname(long userId, long friendId, String nickname) throws RemoteException {
        String sql = "UPDATE friendships SET nickname = ? WHERE user_id = ? AND friend_id = ?";
        try (Connection conn = Database.getConnection();
